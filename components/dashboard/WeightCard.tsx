...
export function WeightCard({ weight }: { weight?: number | null }) {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
      ...
      <p className="mt-1 text-2xl font-semibold text-white">{weight ? `${weight} lbs` : "Not logged yet"}</p>
      <div className="mt-4 flex gap-2">
        <Input value={value} onChange={(e) => setValue(e.target.value)} placeholder="161" type="number" />
        <Button onClick={() => { if (value) { saveWeight.mutate(Number(value)); setValue(""); } }}>Save</Button>
      </div>
    </div>
  );
}
