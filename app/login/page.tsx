  ...
  return (
    <main className="suhu-page flex min-h-screen items-center justify-center px-5 py-10">
      <section className="w-full max-w-md rounded-[2rem] suhu-card p-6">
        <div className="mb-8 flex h-14 w-14 items-center justify-center rounded-3xl bg-[#c8f065] text-[#0a0a0a]"><Dumbbell className="h-7 w-7" /></div>
        ...
        <div className="mt-8 space-y-3"><Input value={pin} onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()} type="password" inputMode="numeric" maxLength={8} placeholder="PIN" className="text-center text-2xl tracking-[0.6em]" /><Button className="w-full" size="lg" onClick={submit}>Unlock</Button>{error ? <p className="text-sm text-red-400">{error}</p> : null}</div>
      </section>
    </main>
  );
}
