  ...
  try {
    ...
    if (!process.env.APPLE_HEALTH_SYNC_SECRET) {
      return NextResponse.json(
        { error: "APPLE_HEALTH_SYNC_SECRET is not configured." },
        { status: 500 }
      );
    }
    ...
    const dailyLogPayload = {
      ...
      active_calories_source:
        body.active_calories == null ? "manual" : "apple_health",

      resting_heart_rate: numberOrNull(body.resting_heart_rate),
      resting_heart_rate_source:
        body.resting_heart_rate == null ? "manual" : "apple_health",

      average_heart_rate: numberOrNull(body.average_heart_rate),
      average_heart_rate_source:
        body.average_heart_rate == null ? "manual" : "apple_health",
    ...
    if (workouts.length > 0) {
      const workoutPayload = workouts
        ...
        .map((workout) => ({
          apple_uuid:
            workout.apple_uuid ??
            `${workout.date}-${workout.workout_type}-${workout.started_at ?? "unknown"}`,
          date: workout.date,
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Apple Health sync error.";
