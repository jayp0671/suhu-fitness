...
export default function DashboardPage() {
  ...
  const planDone = exercises.filter((item) => item.is_done).length;
  ...
  return (
    <main className="suhu-page px-5 pb-32 pt-8">
      <div className="mx-auto max-w-md">
        <PageHeader
          title="Today"
          subtitle="Tiny wins, clean logs, and no guessing."
        />
        ...
        {today.isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <RingProgress
                label="Water"
                value={Number(log.water_oz ?? 0)}
                goal={GOALS.water}
                unit="oz"
              />
              <RingProgress
                label="Steps"
                value={Number(log.steps ?? 0)}
                goal={GOALS.steps}
                unit=""
              />
              <RingProgress
                label="Sleep"
                value={Number(log.sleep_hrs ?? 0)}
                goal={GOALS.sleep}
                unit="h"
              />
              <RingProgress
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-neutral-400">
                    {todayPlan.exercises.length
                      ? `${planDone}/${todayPlan.exercises.length} exercises done`
                      : "Rest day. Recovery counts."}
                  </p>
                </div>

                <Button asChild>
                  <Link href="/workout">Open</Link>
                </Button>
              </div>
            </div>
