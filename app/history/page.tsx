  ...
  return (
    <main className="suhu-page px-5 pb-32 pt-8">
      <div className="mx-auto max-w-md space-y-4">
        <PageHeader title="History" subtitle="Newest days first, with daily summaries." />
        {isLoading ? (
          ...
          logs.map((log: any) => (
            <HistoryDayCard
              key={log.id}
              log={log}
              meals={(data?.meals ?? []).filter((item: any) => item.date === log.date)}
              exercises={(data?.exercises ?? []).filter((item: any) => item.date === log.date)}
            />
          ))
        ) : (
